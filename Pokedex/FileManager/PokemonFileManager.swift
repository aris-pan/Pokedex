...
class PokemonFileManager {
  ...
  func load() -> Set<Pokemon> { fatalError() }
}

final class PokemonFileManagerLive: PokemonFileManager, ObservableObject {
  private let fileName = "pokemon.json"
  
  override func save(objects: Set<Pokemon>) {
    FileManager.endcodeAndSave(objects: objects, fileName: fileName)
    
    print("Live: Save files count is \(objects.count)")
  }
  
  override func load() -> Set<Pokemon> {
...
final class PokemonFileManagerPreview: PokemonFileManager, ObservableObject {
